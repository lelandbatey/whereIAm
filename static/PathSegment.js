...
return function (rawData, index, count) {
	...
	*/
	...
	for (i = 1; i < this.segments.length; i++){
		...
		cur_seg.bearing_difference = cur_seg.bearing - pas_seg.bearing;
	}
	...
	this.getTrainingData = function (){
		...
		for (i = 1; i < this.segments.length; i++){
			...
			var speed = Math.min(max_speed, Math.max(this.segments[i].speed, min_speed));
			speed = translate_domain(min_speed, max_speed, speed, 0, 1);

			var bearing_diff = this.segments[i].bearing_difference;
			bearing_diff = translate_domain(-2*Math.PI, 2*Math.PI, bearing_diff, 0, 1);
			...
			rv[i+'bearing_diff'] = bearing_diff;
			rv[i+'speed'] = speed;
		}
		return rv;
	};
